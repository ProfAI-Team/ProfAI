...
- **Parallel test flake** (credit-service + post-exam-report) — Phase 7 öncesinden devam; beforeEach cleanup eksikliğinden Serializable retry çakışması. Phase 7 işimi blokluyor değil ama Phase 8 veya sprint gap'inde düzeltilmeli. ✅ Kısmen 2026-04-19, `14a536d` — `applyDelta` P2034 (SQLSTATE 40001) için 3 retry + 20/40ms backoff, 4 yeni unit test. Paralel spend race çözüldü. Kalan bug: `tests/globalSetup.ts` pgvector migration `CREATE EXTENSION IF NOT EXISTS vector;` public'te no-op olunca worker schema'da phase-7 migration `vector(768)` type'ı bulamıyor → test_worker_2+ schema'ları eksik migrate → paralel post-exam-report P2003 FK violation. Phase 8 başı: migration SQL'e `WITH SCHEMA public` fix veya phase-7 migration'da `public.vector(768)` tam nitelikli type.
