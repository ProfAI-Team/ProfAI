...
- **App.tsx route mount order** — dnaRoutes + multimodalRoutes global `router.use(authenticate)` Phase 7 b2b router'dan ÖNCE mount edildiğinde public marketplace browse + tutor detail endpoint'leri 401 alıyor. Mount order'ı b2b sonrasına taşıdım; smoke 6/6'ya çıktı. Bu class'ı Phase 6 `/api/health` 401 bug'ının kardeşi. ✅ 2026-04-19 `35fc827` — dnaRoutes 16 endpoint'ine explicit `authenticate` middleware eklendi, global `router.use(authenticate)` kaldırıldı (multimodalRoutes zaten per-route idi). app.ts mount order artık load-bearing değil. Smoke 10/10 yeşil, DNA + B2B integration regression yok.
- **Frontend /tutors auth gate eksik** — TutorListPage auth kontrol etmeden tutorService.match() çağırıyor; logged-out ziyaretçi için 401 → login redirect. Smoke sırasında keşfedildi, kritik değil (demo akışı login zaten gerektiriyor) ama Phase 8'de `isAuthenticated` gate eklemek doğru olacak. ✅ 2026-04-19, `2755ce1` — `optionalAuthenticate` middleware + service-layer public preview (studentId null → embedding skip, classical rubric intact); logged-out visitor "Giriş yap, DNA uyumunu gör" banner'ı ile akışta kalıyor.
- **Parallel test flake** (credit-service + post-exam-report) — Phase 7 öncesinden devam; beforeEach cleanup eksikliğinden Serializable retry çakışması. Phase 7 işimi blokluyor değil ama Phase 8 veya sprint gap'inde düzeltilmeli. ✅ 2026-04-19 üç katmanlı kapanış: (1) `14a536d` — `applyDelta` P2034 retry wrapper + 4 unit test (paralel spend race ele alındı). (2) `0e94ab1` — beforeEach cleanup `contains` → `startsWith`; post-exam-report'un `report-credit-<ts>@...` emaili credit-service'in "credit-" filtresini tetiklerken (iki suite aynı public schema'da koşuyor çünkü test worker izolasyonu silent no-op) FK violation yaratıyordu. 5x paralel run 14/14 stabil. (3) `8a14f82` — pgvector migration `WITH SCHEMA public` + phase-7 migration'da `public.vector(768)` tam nitelikli tipler; gelecek fresh deploy'larda type resolution deterministic. Arta kalan retro: `tests/globalSetup.ts` migrate deploy'ı worker schema'larını yaratmıyor (test_worker_N bulunmuyor), testler de facto public'te koşuyor; bu schema-isolation drift'i ayrı retro borç olarak dokümante edildi. ✅ 2026-04-19 `a4a6d1d` dördüncü katman — globalSetup + setup.ts başına `import "dotenv/config"` eklendi (vitest `.env`'i otomatik yüklemiyordu, `process.env.DATABASE_URL` undefined kalıp silent bail'a neden oluyordu; PrismaClient constructor'ı kendi `tryLoadEnvs`'i ile public schema URL'ini lazy yüklüyor, testler public'te birikiyordu). Ayrıca globalSetup explicit `CREATE SCHEMA IF NOT EXISTS` (Prisma Client raw exec) + migrate deploy execSync stderr capture — artık silent değil, hata loudly throw eder. Drop → npm test → `\dn` test_worker_1..4 mevcut, 11 migration + 33 tablo; public seed (223 user) intakt; credit-service + post-exam-report 5/5 paralel yeşil; full suite 319/319 (önceki 266 + 47 skipped yerine tamamı koşuyor).
